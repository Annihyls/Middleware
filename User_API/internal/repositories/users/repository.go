...
func UpdateUser(id uuid.UUID, prenom string, nom string) error {
	...
	if &nom != nil {
		_, err = db.Exec("UPDATE users SET note = ?, Description = ? WHERE id=?", prenom, &nom, id.String())
	} else {
		_, err = db.Exec("UPDATE users SET note = ?, Description = ? WHERE id=?", nom, sql.NullString{}, id.String())
	}
